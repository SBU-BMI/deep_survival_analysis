...
def eval(args, config, device):
    ...
    if args.aggfeat == 'avg':
        model = agg_feat.AvgFeat(input_nc=3, n_intervals=n_intervals)
    elif args.aggfeat == 'cat':
        model = agg_feat.CatFeat(input_nc=3, n_intervals=n_intervals)
    else:
    ...
    for idx, data in enumerate(data_loader, 0):
        ...
        imgs, preds = imgs[:, :3, :, :], imgs[:, 3:, :, :]
            
        n = imgs.shape[0]

        for i in range(n):
            model.aggregate_features(imgs[i:i+1], preds[i:i+1])

        # print('{}/{} done!'.format(idx, n_repetitions))
        if (idx + 1) % n_repetitions == 0:
            if feat_level == 'out':
                features = model.get_output()
            else:
            ...
            fn = '{}/feat_level_{}.npy'.format(feat_dir_wsi_id, feat_level)
            np.save(fn, features)
            model.reset_features()
            print('mode {}, feat_level {}, {} {} done!'.format(mode, feat_level, (idx + 1) // n_repetitions, wsi_ids[0]))
