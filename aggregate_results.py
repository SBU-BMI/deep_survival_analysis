...
def eval(args, config, device):
    ...
    res_val = np.zeros((num, 2), dtype=np.uint8)
    for idx, data in enumerate(data_loader, 0):
        imgs, y, obs, wsi_ids = data
        feat_dir_wsi_id = '{}/{}/feat_level_out_pred.npy'.format(feat_dir, wsi_ids[0])
        out = np.load(feat_dir_wsi_id)
        res_val[idx, 0] = y.numpy()[0]
        res_val[idx, 1] = np.argmax(out)
